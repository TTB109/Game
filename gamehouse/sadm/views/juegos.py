...
def lista_videojuegos(request,administrador):
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
            ...
            return render(request,'adm/juegos/videojuegos.html',{'juegos':posts,'myFilter':myFilter,'page':page})    
    ...
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
          ...
          if request.method == 'POST':
            ...
            return render(request,'adm/juegos/editar_videojuego.html',{'VJuegos':vjuego_form,'fImagen':imagen_form})
        else:# Tengo iniciada sesión como jugador normal
            print("No tienes permisos!!")
            raise PermissionDenied # Error 403 forbidden             
    except Jugador.DoesNotExist:
...
def eliminar_videojuegos(request,administrador,id_juego):
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
          try:
            vjuego=get_object_or_404(Juego,id_juego =id_juego)
            vimagen=get_object_or_404(Imagen,id_imagen=id_juegod)
          ...
          if request.method=="POST":
            ...
            return render(request,'adm/juegos/eliminar_videojuegos.html') 
...
def registro_videojuegos(request,administrador):
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
            ...
            return render(request,'adm/juegos/registro_videojuego.html',{'VJuegos':juego_form,'fImagen':imagen_form})
...
def genero_plataforma(request,administrador):
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
            ...
            return render(request,'adm/juegos/generos_plataformas.html',{'fgenero':genero,'fplataforma':plataforma})                
        else:# Tengo iniciada sesión como jugador normal
            print("No tienes permisos!!")
            raise PermissionDenied # Error 403 forbidden             
    except Jugador.DoesNotExist:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
          if request.method == 'POST':
            ...
            if all([genero_form.is_valid(),plataforma_form.is_valid()]):
              ...
              return redirect('genero_plataforma')
          else:
            genero_form = GeneroForm()
            plataforma_form = PlataformaForm()
          return render(request,'adm/juegos/registro_Gen_Pla.html',{'fgenero':genero_form, 'fplataforma':plataforma_form})
    ...
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
            ...
            if request.method=="POST":
              ...
              return render(request,'adm/juegos/eliminar_Plataforma.html')
...
def editar_Plataforma(request,administrador,id_plataforma):
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
          ...
          return render(request,'adm/juegos/editar_Plataforma.html',{'fplataforma':usuario_form})
    ...
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
            ...
            if request.method=="POST":
              ...
              return render(request,'adm/juegos/eliminar_Genero.html')
...
def editar_Genero(request,administrador,id_genero):
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
          ...
          return render(request,'adm/juegos/editar_Genero.html',{'fgenero':genero_form})
    ...
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
            ...
            try:
              ...
              posts=paginator.page(paginator.num_pages)
            return render(request,'adm/juegos/CaractPU.html',{'fvcpu':vcpu,'page':page})
    ...
    try:
        ...
        if user.get_username() == solicitado.username: ## Tengo iniciada una sesión de adm
          ...
          try:
            ...
            posts=paginator.page(paginator.num_pages)
          return render(request,'adm/juegos/CaractDE.html',{'fvcde':vcde,'page':page})
